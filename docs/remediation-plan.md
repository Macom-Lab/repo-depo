...
| --- | --- | --- | --- | --- |
| VT-12: Unfixed Debian base-image advisories | Trivy reports 3 critical and 13 high package records with no available fixed version after the base refresh. They affect Perl and several system libraries; some records repeat one CVE across related packages. VulnTracker does not intentionally invoke most affected features, but future code or an attacker with code execution could reach them. | Rebuild on every base-image update and rescan. Move to a smaller distroless or custom Python runtime if Debian updates do not remove the affected packages. Estimated effort: 1-2 days to prototype and validate a distroless build; minutes for a normal patched rebuild. | CI blocks every fixable high/critical finding. Runtime controls include non-root UID, no Linux capabilities, read-only application filesystem, seccomp, resource limits, ingress restriction, and minimal use of OS tooling. | Platform engineering; rebuild immediately when a fix is published, otherwise reassess in 30 days. |
