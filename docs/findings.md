...
| Repository SCA and secrets | Trivy 0.74.0 | 0 dependency vulnerabilities and 0 exposed secrets |
| Container | Docker Scout 1.24.0 | 36 advisories: 2 critical, 2 high, 4 medium, 25 low, 3 unspecified |
| Container (supplementary) | Trivy 0.74.0 | 136 package records: 3 critical, 13 high, 53 medium, 57 low, 10 unknown; 0 fixable high/critical |
...
| VT-12 | Docker Scout and Trivy / container | **High residual.** The latest Trivy snapshot contains 3 critical and 13 high package records without a fixed version; Docker Scout's earlier snapshot reported 2 critical and 2 high. The records cover Perl and system libraries, and one CVE can appear against several related packages. | A future code path that invokes affected regular-expression, archive, FTS5, terminal, compression, or ACL functionality could affect API availability, confidentiality, or container integrity. Current application paths do not intentionally invoke most affected features, and the pod is non-root and read-only. | Pinned base image | **Accepted temporarily.** Fixable OpenSSL findings were removed by refreshing the base digest; the remaining entries have no vendor fix. See `remediation-plan.md`. |
...
| VT-16 | Trivy / container | **High.** The first Trivy policy run found a fixable OpenSSL denial-of-service vulnerability in three related OS packages in the previously pinned base image. | Leaving a vendor-fixed cryptographic library in the runtime image would unnecessarily expose the service to malformed-input denial of service and make the new CI gate fail. | Pinned base image | **Fixed.** The Python 3.11.16 slim base digest was refreshed to the patched Debian build. The final Trivy gate reports 0 fixable high/critical findings. |
